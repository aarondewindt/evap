...
import { init_memory, State, LocationViewProviderProps } from "./types"
...
const LocationViewContext = createContext<Context>({} as Context);

export const LocationViewProvider = ({ children, ...props }: LocationViewProviderProps) => {
  ...
  return <LocationViewContext.Provider value={context}>
    {children}
  </LocationViewContext.Provider>
}

export const useLocationViewContext = () => {
  const context = useContext(LocationViewContext);

  if (context === undefined) {
    throw new Error("useLocationViewContext must be used within LocationViewProvider");
  }
